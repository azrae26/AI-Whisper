...
**僅做以上，不多餘：**
- 只做 `status` + `diff`，不必 `diff --cached`
- 不必檢查 deploy.ps1 是否存在，直接執行即可
- 不必 Read 原始檔，commit message 一律依 diff 撰寫

### Step 2：執行腳本（1 次工具調用）

```powershell
& "<workspace>\deploy.ps1" -Message "commit message 內容" [-Files "新檔案1","新檔案2"]
```
...
```powershell
& "<workspace>\deploy.ps1" -Message "..." -Files ".cursor/rules/new-rule.mdc","assets/new-icon.png"
```
...
- `<workspace>` 為專案根目錄實際路徑（如 `f:\Cursor\AI Whisper`）
