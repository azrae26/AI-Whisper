---
name: restart
description: 重啟 AI Whisper。收到「重啟」、「restart」、「重新啟動」時觸發，或修改完 .py 程式碼後自動執行。
---
...
**永遠只使用 `restart-with-log.ps1`**，不要直接執行 `py main.py` 或 `pythonw main.py`。
...
確認輸出中有出現 `[main][HH:MM:SS] ✅ 快捷鍵` 等啟動訊息。

**若 log 為空：不代表啟動失敗**，只是程序還沒輸出，不要額外再執行 `py main.py`。

## 觸發時機

以下兩種情況都必須執行重啟：

1. 每次修改完 .py 程式碼後，必須自動重啟 AI Whisper 讓改動生效，不需要等使用者要求。
...
- 使用 PowerShell，不使用 `&&`
- 不要直接執行 `py main.py`，一律透過 `restart-with-log.ps1`
